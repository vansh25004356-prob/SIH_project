...
import { MapTrifold, Waveform, Warning, HardDrives, FileText, Broadcast, ChartLineUp, Buildings, Users, SignOut, PaperPlaneTilt, List, X } from "@phosphor-icons/react";
import { roleFromStorage, setRole } from "@/lib/api";
...
export default function Shell({ children }) {
    ...
    const logout = () => { setRole(""); nav("/login"); };
    ...
    const Sidebar = ({ mobile = false }) => (
        <aside className={`tactical-panel flex flex-col ${mobile ? "w-[80vw] max-w-[280px] h-full" : "h-screen sticky top-0 w-[220px] hidden lg:flex"}`} data-testid={mobile ? "mobile-drawer" : "sidebar"}>
                ...
                {NAV.map(({ to, label, icon: Icon, testid }) => {
                    ...
                    return (
                        <Link
                            ...
                            to={to}
                            data-testid={testid}
                            className={`flex items-center gap-2 px-4 py-2.5 text-sm transition-colors ${active ? "bg-white/5 text-white border-l-2 border-[var(--sev-critical)]" : "text-[var(--text-2)] hover:text-white hover:bg-white/[0.03] border-l-2 border-transparent"}`}
                        >
            ...
            <div className="border-t border-[var(--border)] p-3 space-y-2">
                <Link to="/public" className="block text-xs font-mono uppercase tracking-widest text-[var(--text-2)] hover:text-white" data-testid="link-public">
                    <Users size={12} className="inline mr-1" /> Citizen Portal
                </Link>
                <Link to="/field" className="block text-xs font-mono uppercase tracking-widest text-[var(--text-2)] hover:text-white" data-testid="link-field">
                    <MapTrifold size={12} className="inline mr-1" /> Field Officer
                </Link>
                <div className="text-[10px] font-mono uppercase tracking-widest text-[var(--text-2)] pt-2 border-t border-[var(--border)]">
                    Signed in as <span className="text-white">{role || "GUEST"}</span>
                </div>
                <button onClick={logout} data-testid="btn-logout" className="w-full text-left text-xs font-mono uppercase tracking-widest text-[var(--text-2)] hover:text-white flex items-center gap-1">
        ...
        <div className="min-h-screen flex" data-testid="app-shell">
            ...
            <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-[var(--bg)] border-b border-[var(--border)] px-3 py-2 flex items-center gap-3">
                ...
                <Link to="/dashboard" className="flex items-center gap-2">
                    <div className="w-7 h-7 flex items-center justify-center bg-[var(--sev-critical)] text-white font-heading font-bold text-xs">NS</div>
                    <div className="font-heading font-bold tracking-tighter text-sm leading-tight">NER-SLIDE</div>
                </Link>
