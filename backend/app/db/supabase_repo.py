...
async def list_sensors(status: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    for x in res.data or []: out.append({"sensor_id": x["sensor_id"], "zone_id": x.get("zone_id"), "type": x["sensor_type"], "status": x["status"], "lat": x["lat"], "lon": x["lon"], "metadata": x.get("metadata") or {}, "last_seen_iso": x.get("last_seen_at")})
    return out
