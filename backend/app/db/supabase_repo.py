...
def _zone(row: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "id": row.get("id"), "zone_id": row.get("zone_id"), "name": row.get("name"),
        "district": row.get("district"), "state": row.get("state"), "centroid": row.get("centroid") or {},
        "geometry": row.get("geometry"), "population": row.get("population"),
        "terrain": {k: row[k] for k in ("elevation_m", "slope_deg", "aspect_sin", "aspect_cos", "curvature_1_m") if row.get(k) is not None},
        "terrain_source": row.get("terrain_source"), "road_blocked": row.get("road_blocked", False),
        "isolated_villages": row.get("isolated_villages", 0), "recent_field_report": row.get("recent_field_report", False),
        "created_at": row.get("created_at"), "updated_at": row.get("updated_at"),
    }


async def list_zones(state: Optional[str] = None) -> List[Dict[str, Any]]:
    c = await client(); res = await c.rpc("list_zones_geojson", {"p_state": state}).execute()
    return [_zone(x) for x in (res.data or [])]


async def get_zone(zone_id: str) -> Optional[Dict[str, Any]]:
    c = await client(); res = await c.rpc("get_zone_geojson", {"p_zone_id": zone_id}).execute()
...
async def nearest_roads(lat: float, lon: float, limit: int = 3) -> List[Dict[str, Any]]:
    c = await client(); res = await c.rpc("nearby_roads", {"p_lat": lat, "p_lon": lon, "p_limit": limit}).execute()
    return [{"road_id": x["road_id"], "name": x["name"], "status": x["status"], "geometry": x["geometry"], "distance_km": x["distance_km"]} for x in (res.data or [])]
...
async def list_sensors(status: Optional[str] = None) -> List[Dict[str, Any]]:
    c = await client(); res = await c.rpc("list_sensors_geojson", {"p_status": status}).execute(); out = []
    for x in res.data or []:
        out.append({"sensor_id": x["sensor_id"], "zone_id": x.get("zone_id"), "type": x["sensor_type"], "status": x["status"], "lat": x["lat"], "lon": x["lon"], "metadata": x.get("metadata") or {}, "last_seen_iso": x.get("last_seen_at")})
    return out
...
async def insert_sensor_reading(sensor_id: str, measurement_type: str, value: float) -> Dict[str, Any]:
    ...
    now = datetime.now(timezone.utc).isoformat(); row = {"sensor_id": sensor.data["id"], "measurement_type": measurement_type, "value": value, "recorded_at": now}
...
async def insert_report(payload: Dict[str, Any]) -> Dict[str, Any]:
    c = await client()
    row = {
        "client_uuid": payload.get("client_uuid"), "reporter_id": payload.get("reporter_id"),
        "lat": payload["lat"], "lon": payload["lon"], "report_type": payload["report_type"],
        "description": payload.get("description") or "", "reporter_role": payload.get("reporter_role", "CITIZEN"), "status": "SUBMITTED",
    }
...
async def insert_report_media(report_id: str, media: Dict[str, Any]) -> Dict[str, Any]:
    c = await client()
    row = {"report_id": report_id, "storage_path": media["storage_path"], "media_type": media.get("media_type", "PHOTO"), "mime_type": media.get("mime_type"), "size_bytes": media.get("size_bytes")}
    res = await c.table("report_media").insert(row).select("*").single().execute(); return dict(res.data)
...
async def create_alert(payload: Dict[str, Any]) -> Dict[str, Any]:
    c = await client(); res = await c.table("alerts").insert(payload).select("*").single().execute(); return dict(res.data)
...
async def create_feedback(payload: Dict[str, Any]) -> Dict[str, Any]:
    c = await client(); zone = await c.table("zones").select("id").eq("zone_id", payload["zone_id"]).single().execute()
    row = {"zone_id": zone.data["id"], "label": payload["label"], "notes": payload.get("notes") or "", "created_by": payload.get("created_by")}
