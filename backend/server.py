...
MongoDB is no longer used by the API. Supabase/Postgres is the persistence
layer; PostGIS RPCs provide GeoJSON and nearest-neighbour queries. The V5 ML
model remains local to the backend and only its inputs/outputs are persisted.
"""
...
async def gis_alerts() -> List[Dict[str, Any]]:
    return await repo.list_alerts(50)
...
async def notification_status() -> Dict[str, Any]:
    configured = bool(os.environ.get("TWILIO_ACCOUNT_SID") and os.environ.get("TWILIO_AUTH_TOKEN") and os.environ.get("TWILIO_FROM_NUMBER"))
