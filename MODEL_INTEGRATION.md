...
Operational precision on real deployment will differ — apply `recalibrate()` with a real
prevalence estimate before setting an operational cutoff (see report `known_limitations_still_present`).
...
```
The risk service:
1. Fetches the zone document from Mongo (with its stored `terrain` block).
...
6. Stores the result in `risk_predictions` collection.
